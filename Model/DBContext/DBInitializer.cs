namespace Fitness.Model.DBContext
{
    ...
    public class DBInitializer : CreateDatabaseIfNotExists<FitnessDB>//CreateDatabaseIfNotExists<FitnessDB>
    {
        ...
        {
            //System.Console.WriteLine(context.Clients.Count());
            //System.Console.WriteLine(context.Entries.Count());
            //System.Console.WriteLine(context.Tickets.Count());
            //System.Console.WriteLine(context.Users.Count());
            //System.Console.WriteLine(context.TicketTypes.Count());
            Client client = context.Clients.ToList().Where(c => c.Id == 1).First();
            Client client2 = context.Clients.Where(c => c.Id == 2).First();

            TicketType type = context.TicketTypes.Where(tt => tt.Id == 1).First();
            User seller = context.Users.Where(u => u.Id == 1).FirstOrDefault();
            
            //Client client = context.Clients.Local[1];//context.Clients.Where(c => c.Id == 1001).First();
            //Client client2 = context.Clients.Local[2];
            //TicketType type = context.TicketTypes.Local[0];
            //User seller = context.Users.Local[0];
        ...
        private void AddEntries(FitnessDB context)
        {
            ...
            {
                context.Entries.Add(new Entry { UserTicket = ticket, UserTicketId = ticket.Id, BarCode = client.BarCode, LoginTime = new System.DateTime(2019, 4, 1, 18, 5, 32), Inserter = inserter, InserterId = inserter.Id, TrainingType = "Fitness" });
                context.Entries.Add(new Entry { UserTicket = ticket, UserTicketId = ticket.Id, BarCode = client.BarCode, LoginTime = new System.DateTime(2019, 4, 2, 18, 4, 11), Inserter = inserter2, InserterId = inserter2.Id, TrainingType = "Fitness" });
